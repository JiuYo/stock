        ...
        plus.runtime.getProperty(plus.runtime.appid, function (inf) {
            ...
            mui.ajax({
			    ...
			    success: function(data) {
			    	...
			    	{
			    		androidUpdate(data.Android,ismanual,ver);
			    	}
			    	else
			    	{
			    		iosUpdate(data.iOS,ismanual,ver);
			    	}
			    },
 ...
 */
function androidUpdate(data,ismanual,oversion)
{
	var nversion = data.version;
	if(compareVersion(oversion,nversion))
	{
		//有新版本，需要更新
 ...
 * @param {Boolean} ismanual 是否手动更新，true表示手动更新，需要弹出来多提示
 */
function iosUpdate(data,ismanual,oversion)
{
	var nversion = data.version;
	if(compareVersion(oversion,nversion))
	{
 ...
 * @param {Object} url
 */
function downloadinstallapk(url)
{
...
/**
 * 服务器入参
 * type="android/ios" 需要传入设备类型,有两种,分别为android或ios
 * 返回参数
	{
	    "appid":"HelloH5",
	    "iOS":{
